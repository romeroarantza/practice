import java.util.*;
//encapsulate the data entry to a method
...
public class BowlingTeamDemo4
{
   public static void main(String[] args)
   {
      ...
      BowlingTeam[] teams = new BowlingTeam[4];
      getTeamData(teams);
      ...
      for(y = 0; y < NUM_TEAMS; ++y)
      {
       ...
       boolean isValidTeam = false;
        ...
        for(y = 0; y < teams.length && !isValidTeam; ++y)
        {
            isValidTeam = name.equals(teams[y].getTeam());
            if(isValidTeam)
            {
               for(x = 0; x < NUM_TEAM_MEMBERS; ++x)
                  System.out.print(teams[y].getMember(x) + " ");
     ...
     //searching the array for a match 
          System.out.println();
   }
